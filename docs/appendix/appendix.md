...
内積カーネルを SIMD 化したのに、全探索の見かけの帯域が 5.7 GB/s と低く、カーネル単体も 167ns で頭打ちでした。SIMD からスカラへ戻る境界に `VZEROUPPER` を 1 命令置くだけで 23ns(7.1x)まで速くなり、見かけの帯域の上限も消えました。
