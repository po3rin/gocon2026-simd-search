...
一方、SIMD ループの直後にある水平和 `sum = buf[0]+buf[1]+…` はスカラの float32 計算で、Go はこれをレガシー SSE 命令(下位 128bit しか知らない古い命令)で出力します。Intel の CPU は、上位が dirty なままレガシー SSE を実行すると、上位の値を壊さないための処理を命令ごとに挟みます。これがペナルティの正体です。`VZEROUPPER` は上位 128bit をゼロに掃除する 1 命令で、境界で一度呼べばこのペナルティは消えます。命令自体のコストは小さいので、差し引きで大きく得をします。
