...
何が起きているかを次の図にまとめました。前提になるレジスタの構造、境界の 1 命令だけが違う実行の流れ、時間の内訳、の順に見てください。

![VZEROUPPER の遷移ペナルティ: レジスタの構造、実行の流れ、時間の内訳](../images/vzeroupper.png)

原因は YMM レジスタの上位半分です。YMM レジスタ(256bit)は、下位 128bit を古い XMM レジスタと共有しています。AVX2 命令(`Float32x8` の FMA など)は 256bit 全体に書くので、実行後は上位 128bit に値が残ります。この状態を dirty(汚れた)と呼びます。

一方、SIMD ループの直後にある水平和 `sum = buf[0]+buf[1]+…` はスカラの float32 計算で、Go はこれをレガシー SSE 命令(下位 128bit しか知らない古い命令)で出力します。Intel の CPU は、上位が dirty なままレガシー SSE を実行すると、上位の値を壊さないための処理を命令ごとに挟みます。この追加処理がペナルティになります。`VZEROUPPER` は上位 128bit をゼロに掃除する 1 命令で、境界で一度呼べばこのペナルティは消えます。命令自体のコストは小さいので、差し引きで大きく得をします。
