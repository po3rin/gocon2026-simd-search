...
Go 1.27 の標準 SIMD(`simd/archsimd`)を使い、外部ライブラリなしの Pure Go でベクトル検索を高速化する教材です。題材は内積によるベクトル検索です。ただ速くするのではなく、ルーフラインモデルで「いま何が性能の上限になっているか」を測ってから打つ手を選んでいきます。
...
まず SIMD が無いとどうなるか見ていきましょう。次の Go コードはベクトルの内積です。
...
計算の本体は「内積を 10 万回計算する」ことです。内積は掛け算と足し算の塊なので、SIMD が得意とする部分です。世のベクトル検索エンジン(Faiss、Qdrant、ClickHouse など)も、内積などの距離計算を SIMD で実装しています。それを Pure Go で追体験するのが今回の趣旨です。本ワークショップの題材カーネルは `acc += a[i]*d[i]`(クエリ a と DB ベクトル d の内積)で、これを 10 万本ぶん回します。
...
そこで本ワークショップは、速くする手をいきなり打たず、先に「何で詰まっているか」を測って見極めてから打ち手を選びます。そのための道具がルーフラインモデルです。
...
どんな最適化もこの線より上には行けません。ここに自分のコードの達成性能(GFLOP/s)と算術強度を当てはめれば、2 つの上限のどちらに近いか、すなわちメモリ律速か演算律速かが分かります。メモリ律速なら「算術強度 を上げる」、演算律速なら「実装効率を上げて演算ピークに近づける」というように、打つ手が決まります。
...
1 と 2 がルーフラインの式に入れる 2 つの数字です。今回の検索は DB ベクトルを読むだけで、大きな配列への書き込みはしません。そのためメモリ帯域には 2 の読むだけの値を使い、3 は他の資料の数字と見比べるための参考値です。まずは走らせます:
...
3 つの数字が出ました。これがこのマシンの性能の上限です(演算ピーク 約 25.5 GFLOP/s、メモリ帯域 約 20 GB/s)。では、この数字を出しているコードが何をしているのかを順に見ます。測り方を知って初めて、数字を信用できるからです。
...
実測は **25.5 GFLOP/s** です。試しにアキュムレータを 4 本に減らした `BenchmarkPeakFLOP_AVX2_4acc` を測ると 13.4 GFLOP/s まで落ちます(本数を減らすと遅くなる)。理論ピークは約 100 GFLOP/s で、実測はその約 1/4 です。Go のコード生成(1.26 / 1.27 とも)がこのループのアキュムレータを毎回スタックへ退避する(register spill)ためですが、検索は 算術強度 0.5 の深いメモリ律速なので、この低さは結論を変えません。

> **コラム: register spill を見る(任意):** 実測 25.5 が理論の約 100 に届かないのは、Go のコード生成(1.26 / 1.27 とも)がこのループでは SIMD 値をレジスタに保てず、12 本のアキュムレータを毎回スタックへ退避するためです。これを register spill と呼びます。本数が多すぎるのではありません。12 本に m と c を足した 14 本は、使える 15 本の Y レジスタに収まります。Y15 は Go 内部 ABI の予約ゼロレジスタで割り当てられないため 15 本です([golang/go#76969](https://github.com/golang/go/issues/76969))。本当に起きているかは `make spill` で確かめられます。演算ピークループ `BenchmarkPeakFLOP_AVX2` の機械語をコンパイラの `-S` 出力で見るもので、amd64 クロスコンパイルなので mac でも動きます。各アキュムレータ `aN` がロード、FMA、ストアの三つ組を毎回踏んでいるのが、レジスタに居続けられていない証拠です:
>
> ```text
...
> ```
>
> レジスタに収まっていれば本体は `VFMADD` だけになります。その前後に必ず付く `VMOVDQU …(SP)` が spill です。この spill が、演算ピークが理論ピークの約 100 に届かず 25.5 GFLOP/s に留まる主因とみられます。ただし spill を消せば理論ピークに届くかは、本ワークショップでは検証していません。それでも打ち手は変わりません。検索は 算術強度 0.5 のメモリ律速なので、上限がこの低さでも結論は同じです。なぜ 0.65 FMA/cyc なのかというポート圧まで踏み込むなら llvm-mca や uiCA の領域で、本編の 40 分には含めません。arm64 の Neon でも同じ退避が起きます。Apple M3 Pro で 12 本の FMLA ループ `BenchmarkPeakFLOP_NEON` は 32 GFLOP/s に留まり、`-S` を見ると各 FMLA の前後に `FMOVQ …(SP)` が付きます。アーキが変わっても、Go の SIMD コード生成がまだ発展途上である点は共通です。
...
CPU チップの理論値はもっと上です(この機で AVX2 約 100。AVX-512 はこの CPU には無い)が、いま動かしている Go コードでは 25.5 が現実的な上限です。ルーフライン図には理論値も載せますが、ここではまず実測の 25.5 と 20 を使います。

**内積はどちらに引っかかるか**

内積の算術強度は 算術強度 = 0.5 でした。これは「1 バイト読むごとに、計算は 0.5 回しかない」という意味で、かなり少ない部類です。
...
- 算術強度 が 1.3 より小さいなら、計算が足りないのではなく、データの読み込み待ち(メモリ律速)
- 算術強度 が 1.3 より大きいなら、計算の処理能力が上限(演算律速)
...
さきほど測った上限と比べながら、改良を 1 つずつ足して速さを確かめていきます。下の図は、その過程をルーフラインに重ねたものです(横軸 = 算術強度、縦軸 = 性能、斜線 = メモリ帯域の上限、水平線 = 演算ピーク)。点がどちらの上限に近いか(メモリ律速か演算律速か)を、これから 1 つずつ見ていきます。

![このマシンの実測ルーフライン全体像](../images/roofline-plot.png)

図: スカラ、SIMD(メモリ帯域の上限)、カーネル単体(演算側)、int8(リッジの右)、1bit 量子化(右上へ)。各点の詳細は下の各節で。

> **この図を自分の実測で動かす(任意):** `make roofline-plot` を実行すると、いま測った数字から下のような対話ルーフライン(`/tmp/roofline.html`)が出ます。各点から上限へ点線を引いて「上限の何 % か」を出すので、Stage 1 がメモリ帯域の上限に達し、バッチ化でリッジを越えて演算側へ動くのが分かります(点にカーソルを当てると 算術強度・GFLOP/s・達成率が出ます)。
>
> ![make roofline-plot が出す対話ルーフライン(EPYC 7763 実測の例)](../images/roofline-plot-example.png)
>
> **この図の読み方(点がどちらの上限に近いかで、打つ手が決まる):**
>
> - 上限のほぼ 100% に達している点はメモリ律速。実装を磨いても動かない(Stage 1 の結論)。打つ手は 算術強度 を上げること(再利用としてのバッチ、バイト削減としての量子化。Stage 2・3 で実測する道)。
> - リッジを越えて演算側に乗った点(例: SIMD batch)。ここで初めて SIMD がスカラより明確に速くなる(Stage 2 で 5.9x を実測)。
>
> どちらの軸を攻めるかをこの 1 枚が決めます。それが §05 の codegen ビュー(spill 等)との違いです。
>
> 細かいこと: 上限は `make roofline-plot PEAK=<GF> BW=<GB/s>` で渡します(値は `make roofline-ceiling` のもの)。理論ピーク線は `TPEAK=100` です。作図も解析も依存ゼロの Pure Go です。Stage 4 の 1bit 量子化は点として出ません。popcount で flop が無くなり、縦軸 GFLOP/s も横軸 flop/byte も定義できないためです。効果は Stage 4 の静止図と `make roofline-decompose` で見ます。Stage 3 の int8 は 算術強度 2 の点として置けます。52% から上限へ詰める手は未測定なので触れません。上限自体が spill で 25.5 止まりです。
...
make bench-parallel  # 寄り道: goroutine 並列はどの上限に効くか
...
各節に出てくる GFLOP/s と 算術強度 は、これらのベンチが自動で計算して表示します(特別なプロファイラは不要です。算出の詳細は `internal/index/bench_test.go` にありますが、本編では検索コードに集中します)。

### Stage 0 — スカラ基準(ベースライン)

**なぜ:** 最適化はまず基準値からです。素朴に「1 要素ずつ」内積を回し、達成性能と 算術強度 を測ります。以降の手が効いたかは、この基準からの変化で判断します。検索は全ベクトルと内積して上位 k 件を返すだけです。
...
**どうなったか:** 全探索 35.7 ms、**2.15 GFLOP/s**、算術強度 0.5。メモリ帯域の上限(約 10 GF)にすら届いていません。なぜ上限未満かというと、依存連鎖で律速しているからです。`sum += ...` は前の足し算が終わるまで次へ進めず、待ち時間(レイテンシ)がそのまま表に出ます。加算の依存連鎖が直列で、命令レベルの並列性がほぼ活かせません(384 要素 × 加算レイテンシ(Zen3 で約 3 サイクル。サイクル = CPU クロックの 1 拍)÷ 周波数 が、実測のカーネル 348ns とほぼ一致します)。

**次の一手:** ルーフラインの「どの上限にも未達なら命令並列性を上げる」に従い、同時に複数を処理する SIMD で命令並列性を上げ、達成性能を引き上げます。
...
**なぜ:** Stage 0 は命令並列性ゼロで上限にも未達でした。ルーフラインの指示は「命令並列性を上げて達成性能を上げる」です。SIMD 化では 2 つの工夫を一度にやりがちなので、混乱しないよう 1a「8 個まとめて読む」、1b「待ち時間を隠す」の順に分けて理解します。
...
そこで独立したアキュムレータを acc0 と acc1 の 2 本に分けます。互いに依存しないので、acc0 の FMA が計算中でも acc1 の FMA を走らせられ、待ち時間を隠せます。これを命令レベル並列性と呼びます。最後に 2 本を足してから水平和します。仕上げに、SIMD からスカラ計算へ戻る境界で `archsimd.ClearAVXUpperBits()` を呼びます。中身は VZEROUPPER という 1 命令で、Intel 機での遷移ペナルティ対策です。AMD では不要ですが無害です。Go は 1.27 でも自動挿入しないので、標準 API で自分で呼びます。
...
**どうなったか:** カーネル単体は 348 ns から 55 ns(6.3x)、全探索も 35.7 ms から 7.9 ms(4.5x)速くなりました。SIMD は確かに効いています。ただし見るべきは「どこで止まったか」です。全探索の達成は **9.7 GFLOP/s**。これは §05 で見積もったメモリ帯域の上限 0.5 × 約 20 ≈ 10 GFLOP/s の直下で、帯域に直すと 19.4 GB/s、メモリ帯域の上限(約 20)の約 97% です。算術強度 0.5 のまま、メモリ帯域の上限に達しました(共有 VM のばらつきで上限もベンチも ±5% 動くので、9 割を超えていれば「上限に到達」と読みます)。

ここの倍率 4.5x は「naive が上限からどれだけ遠かったか」で決まる量で、CPU によって変わります(naive が既に上限の近くにいる機械なら 1.x 倍程度、遠い機械なら数倍)。変わらないのは「最後はメモリ帯域の上限で頭打ち」という事実のほうです。あなたの Codespace では何倍か、`make bench1` で確かめてください。

**結論:** もう実装効率では伸びません。幅を 8 から 16 の AVX-512 に広げても上限は動かず、キャッシュブロッキングも効きません。DB ベクトルは各 1 回しか読まれず再利用が無いので、タイリングしても DRAM 読み出し総量は変わらないからです。次は実装効率ではなく、算術強度 を上げる番です。算術強度 を上げる道は 2 つあります。①再利用(クエリのバッチ化。結果は exact のまま)と ②バイト削減(量子化。結果は近似になる)です。まず①を Stage 2 で、続いて②を Stage 3 の int8 と Stage 4 の 1bit の 2 段で扱います。

上のコードは要点です。長さガードと「8 幅の端数処理」を含む完全版は `internal/vec/dot_simd.go` にあります。

> **コラム: 端数はマスク付きロードでも書ける(任意):** 上のコードは 8 の倍数に満たない端数をスカラループで処理しています(384 次元は 16 で割り切れるので、実は一度も通りません)。Go 1.27 には端数専用の `LoadFloat32x8Part`(足りないレーンをゼロ埋めして読むマスク付きロード。1.26 では `LoadFloat32x8SlicePart` という名前でした)があり、端数もベクトルのまま処理できます。端数処理を標準 API だけで書けます。
...
> ```
>
> このパッケージには `GODEBUG=simd=128` のように幅を狭めて実行する仕組みがあります。同じバイナリで「256bit(8 レーン)」と「128bit(4 レーン)」の全探索を測り比べられます。Stage 1 の結論は「幅を 8 から 16 に広げても上限は動かない」でした。狭めた場合は次のようになります:
>
> ```bash
...
> ```
>
> 読み方: 256bit ではポータブル版も archsimd 版と同じ点に乗ります。API が変わってもルーフライン上の位置は同じです。幅を 128bit に半分にすると、カーネルは 57 ns から 107 ns と約 2 倍遅くなり、全探索も 8.5 ms から 11.9 ms と遅くなりました。これは 1 ベクトルあたりの時間で読めます。DB ベクトル 1 本は 1536 byte で、DRAM から運ぶのに 1536 ÷ 19 GB/s で約 80 ns かかります。256bit のカーネル 57 ns はこの 80 ns の下に隠れるのでメモリ帯域の上限に達しますが、128bit の 107 ns は 80 ns からはみ出すので、律速がメモリからカーネルへ戻ります。**幅は、広げても上限を越えられず、狭めると上限より下に落ちる。**点は上には動けず、下にだけ動ける。これがメモリ律速の意味です。Apple Silicon は元が 128bit なので、下 2 行は同じ数字になります。ポータブル API には Stage 3 の int8 積和(VPMADDWD)や popcount のようなアーキ固有の命令は入っていません。どのアーキでも共通に持てる演算だけです。そのため本編の量子化カーネルは `archsimd` のままです。
...
**なぜ:** 全探索がメモリ律速なのは、DB ベクトル d を読んでも内積 1 回(2 flop / 4 byte)しかしないからです。d を 1 回ロードして B 本のクエリ全部と内積すれば、同じ転送で計算が B 倍になり、算術強度 ≈ 0.5×B になります。B=32 で 算術強度 16 となり、リッジを越えて演算律速側に移ります。そこなら SIMD が効く見込みで、しかも精度はそのまま(exact)です。事実上の GEMM(行列×行列)化で、BLAS や Faiss のバッチ検索が速い理由と同じです。
...
**どうなったか:** B=32 でクエリを束ねると 算術強度 は 0.5 から 16 になり、リッジを越えて演算律速側に移りました。そこでは **SIMD が exact のままスカラより 5.9x 速くなります**。scalar batch 34.2 ms/query に対し SIMD batch 5.79 ms/query で、GFLOP/s は 2.24 から 13.3 です。Stage 1 ではメモリ帯域の上限に達して伸びなかった SIMD が、算術強度 を上げた途端に効きます。1 クエリあたりも 7.9 ms から 5.8 ms と速くなります(B=1 比 1.4x)。精度は一切犠牲にしていません。

spill による 25.5 GF の上限で 13 GF に留まりますが、それでもスカラより 5.9x 速くなります。ここの倍率も CPU 次第です。
...
なぜ反転するのかは、時間の内訳で見ると分かります。ルーフラインは 1 要素あたりの時間を、メモリ時間(運ぶ byte ÷ 帯域)と演算時間(flop ÷ 演算ピーク)に分けて考え、実時間はおおよそ長い方になります。演算時間は 2 flop 固定で全 Stage 共通ですが、バッチ化はメモリ時間だけを 1/32 に縮めます。そのため律速がメモリから演算へ反転します。下は §05 で実測した上限、演算 25.6 GFLOP/s と read 帯域 20.8 GB/s から計算したものです。
...
図: Stage 2 の位置。バッチ化で 算術強度 が 0.5 から 16 と右へ動き、演算律速側に乗った(exact・精度そのまま)。

### 寄り道 — 「goroutine で並列化すればいいのでは？」

Go を書く人なら気になるところです。ここまで全部 1 コアで測ってきましたが、コアに分ければ速くなるのではないか。実測で答えます。DB を workers 個のチャンクに分けて goroutine で分担し、最後に top-k をマージします(`internal/index/parallel.go`):
...
- メモリ律速(B=1)はマシン全体の帯域で止まります。合算帯域が 16、27、30 GB/s と飽和し、2 から 4 workers ではもう +10% しか伸びません。1 コアの上限(約 20 GB/s)の少し先に、マシン全体の DRAM 帯域というさらに硬い上限があり、コアはそれを取り合うだけです。**メモリ律速にコアを足しても上限は動きません。**
- 演算律速(バッチ)は物理コア数で止まります。この Codespace の 4 vCPU は物理 2 コアに SMT で 2 スレッドずつ載せたもので、`lscpu` で確認できます。SMT は 1 つの物理コアを 2 つの CPU として見せる仕組みで、Intel の Hyper-Threading と同じものです。SMT の兄弟スレッドは FMA などの実行ユニットを共有するので、演算律速の仕事は物理コア数の 2 ぶんまでしか増えず、1.9x で止まります。物理 4 コア以上の機械なら、こちらはコア数なりに伸びます。メモリ律速側は伸びません。

並列化は万能の追加ブーストではなく、メモリ律速なら「マシン全体の帯域」、演算律速なら「物理コア数」という、1 コアのルーフラインの外側にある別の上限に達します。「何律速か」を先に知っていれば、goroutine を足して効くかどうかは、足す前に予測できます。

### Stage 3 — int8 量子化(もう一つの道：バイトを削る・その1)

**なぜ:** Stage 1 のメモリ帯域の上限を越える道は 2 つありました。①再利用は Stage 2 で実測済みで、ここから②バイト削減に入ります。ルーフラインの指示は同じく「算術強度 を上げる」ですが、今度は再利用ではなく、データ表現を変えてバイトを削ります。まず fp32(4 byte)を int8(1 byte)に量子化します。転送が 1/4 になり、算術強度 は 0.5 から 2 flop/byte になって、リッジ(1.3)を越えて演算律速側に乗る見込みです。int8 の演算は整数ですが、数え方は flop と同じで、図では Gop/s と表記します。距離カーネルは整数の積和なので、量子化しても **SIMD が主役のまま**です。
...
int8 同士の積は最大 127×127 で int32 に余裕で収まるので、飽和対策なしの短いコードで済みます。1 命令で 16 要素と、fp32 の 8 要素の 2 倍幅です。arm64 の Neon には VPMADDWD 相当の命令が無く、`int8_arm64.go` は SMULL で掛けて int16 に広げ、SXTL で int32 に広げてから足す 3 段で同じ計算をします。どの命令があるかがアーキごとに違う、それが `archsimd` をアーキ固有と呼ぶ理由です。
...
ただし、転送を 1/4 にしたのに速度は 4x になっていません(達成 9.2 GB/s はメモリ帯域の上限のずっと下)。算術強度 2 でリッジの右へ移った、つまり律速がメモリからカーネルの演算へ移動したからです。上限を 1 つ越えると、次の上限が現れる。ルーフラインの点の位置がそのまま説明になります。
...
**なぜ:** int8 で 1/4 にしても、10 万件 × 384 byte = 38.4 MB は L3 に収まりません。②バイト削減の極限として符号 1bit まで減らせば、1 ベクトル 1536 byte が 48 byte(**1/32**)、全体 4.8 MB でキャッシュに全部乗ります。距離は内積をやめ、XOR+popcount のハミング距離(違うビット数)にします。1bit の利得は転送 1/32 だけではありません。距離カーネルも popcount(1 命令で 64 次元)に変わってはるかに軽くなり、int8 で現れたカーネルの上限も同時に越えられます。
...
**どうなったか:** 153MB が 4.8MB になってキャッシュに乗り、DRAM 帯域の制約から外れました。0.77 ms、**46x** です。速さの源泉は SIMD ではなく「データを 1/32 にした」ことです。距離計算はスカラ POPCNT(64 次元/命令)で足り、ここで AVX-512 の VPOPCNT を使っても速くなりません。量子化後はキャッシュ常駐で popcount 律速ではなく、1 ベクトルも 6 語と小さいからです(AVX-512 機で計測済み: AVX-512 版 `SearchBinarySIMD` 0.75 ms に対しスカラ `SearchBinary` 0.68 ms。なお今回の Codespace の AMD CPU に AVX-512 VPOPCNT は無く、この付録は動きません)。Stage 3(int8)では SIMD が主役だったのに、1bit では使う場所がありません。「計算で詰まっていない所では SIMD は効かない」というルーフラインの一貫した教訓です。本編に AVX-512 ステージを置かないのはこのためです。速くならないうえ全 CPU にあるとも限らないので、本編は AVX2+FMA だけで完結させ、AVX-512 機向けの付録(付録 B)に回します。

ただし、良いことばかりではありません。1bit に減らしたぶん精度が大きく落ち、Recall@10 = 0.18(正解 10 件のうち 2 件弱しか当たらない)。46x は「正確な検索」を速くしたのではなく、近似に問題をすり替えた数字で、このままでは実用になりません(exact な Stage 0/1/2 とは別タスク)。

**次の一手:** int8(0.948)では保てた精度が、1bit では大きく落ちました。速さは保ったまま精度を取り戻します。そこで SIMD が主役として戻ってきます(Stage 5)。
...
これが実運用のベクトル検索の一般的な構成です(量子化で粗く絞り、高精度な距離で rerank する。Faiss / Qdrant / ClickHouse QBit と同系統)。速度(量子化。横)と精度(SIMD 内積。縦)の二本柱で、はじめて「速くて正確」に届きます。
...
本編で使った考え方(ルーフラインと「算術強度 を上げる 2 つの道」)は、そのまま先へ延長できます(MaxSim は付録で実装済み)。方向は 3 つあります:

- **量子化とバッチの合成。** Stage 2(①再利用)と Stage 3/4(②バイト削減)は排他ではありません。int8 化した DB ベクトルをバッチで回せば 算術強度 は掛け算で上がり(int8 × B=32 で 算術強度 ≈ 64)、さらに演算律速側へ進みます。実運用のベクトル検索エンジンが速いのはこの合成のためです。
- **アルゴリズムの軸(HNSW・IVF)。** ここまでの手は全て「10 万件全部に触る」前提でした。そもそも触る件数を減らすのが第 3 の軸で、全探索の O(N) を割る唯一の道です。ただし飛び飛びのメモリアクセスになるため、今度は帯域ではなくレイテンシが上限になります。ルーフラインとは別の性能モデルが要る領域で、実運用は「候補を絞る(HNSW/IVF)」と「絞った中を本ワークショップの技で速く走る」の合成です。
...
本ワークショップがたどったのは「律速が現れる、突破する、次の律速が現れる」の連鎖でした。まずその変遷と、各段での SIMD の役割を 1 枚で振り返ります:

| 遷移 | 当たっていた上限 | 打った手 | SIMD の役割 | 結果 |
|---|---|---|---|---|
| Stage 0→1 | **依存連鎖**(加算レイテンシ) | SIMD 化 + アキュムレータ2本 | **主役**(突破の実行者) | 35.7→7.9 ms(4.5x) |
| Stage 1→2 | **メモリ帯域**(DRAM) | バッチで 算術強度 0.5→16(再利用) | **主役**(演算側でスカラを 5.9x) | 5.8 ms/query・exact |
| 寄り道 | 帯域 / 物理コア | goroutine 並列 | —(並列でも上限は同じ) | 1.8x / 1.9x で頭打ち |
...
6 つの遷移のうち 4 つで SIMD が突破の実行者です。効かない場所(メモリ帯域の上限に達したあと、量子化後の popcount)がはっきりあるからこそ、効く場所の 4 回が際立ちます。その見極めを与えるのがルーフラインでした。実装効率を上げる(SIMD、アキュムレータ)ことと、算術強度 を上げる(バッチ化、量子化)ことは別の軸です。Stage 1 でメモリ帯域に達した時点で「次は実装効率ではなく 算術強度」と手が決まり、算術強度 を上げた先でまた SIMD が効きました。速度と精度も別の軸で、1bit 量子化で落ちた精度は fp32 SIMD の rerank で取り戻しました。

持ち帰ってほしい作法は次の 6 つです:

1.  **ルーフラインで先に上限を見る。** 算術強度 0.5 はリッジ(約 1.3)を大きく下回る。1 行も書く前から「メモリ律速で、本命は 算術強度 を上げること」と分かる
...
算術強度 = Tq/2 = 8 flop/byte がタスク仕様として最初からリッジの右にあり、SIMD が最初から 5.7x 効きます。「算術強度 を上げる工夫(Stage 2)」が、モダンな検索方式では仕様として組み込まれています。
...
**Apple Silicon 参加者向け:** Go 1.27 から `archsimd` が arm64 の Neon(128bit)に対応したので、手元の Mac でも SIMD パスが走ります。`//go:build arm64` の `dot_arm64.go` と `int8_arm64.go` がそれで、上限を測るベンチも Neon 版があります。ただし本編の数字とは別物です。レジスタ幅は 256 から 128bit に半分になる一方、単コアのメモリ帯域は Codespaces より大きいので、点も倍率も本編とは違う位置に打たれます。M3 Pro の実測は次のとおりです。
...
当日は GitHub Codespaces を共有環境にすれば、手元のアーキの違いによらず全員が同じ条件で達成性能と 算術強度 を測れます。Mac の数字は自分の値として持ち帰ってください。

**CPU について:** 本編で使う SIMD は AVX2 + FMA だけです(Stage 1/2/5 の内積。Stage 3 の int8 カーネルは AVX2 のみ)。過去 10 年の x86(Intel Haswell 2013 以降、AMD 2015 以降)がほぼ全て持つため、Codespaces にどの CPU に割り当てられても本編は再現します。AVX-512 の VPOPCNT は本編では使いません。Stage 4 で見たとおり量子化後は速くならないためです。 Faiss も同じ理由で AVX2 と Neon を主に使い、AVX-512 のカーネルは原則持っていません(ダウンクロックで AVX2 より遅くなることがあり、開発とコンパイルのコストも高いため)。AVX-512 を実機で確かめたい人だけ、`infra/` の AWS c7i(Sapphire Rapids)を使ってください。
