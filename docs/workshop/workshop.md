...
> **コラム: 「SIMD が効く境界」はデータサイズの軸にもある(`make bench-nsweep`):** カーネル単体 6.3x と全探索 4.5x の差は、データが L1 に収まるか DRAM から流れてくるかの差でした。DB 件数を変えると、SIMD の倍率がキャッシュに収まらなくなる所で落ちるのが見えます。
>
