...
Recall@10 は精度の指標で、次のように測ります。まず fp32 で正確に検索して上位 10 件を出し、これを正解とします。次に int8 で検索して上位 10 件を出し、そのうち正解に含まれていた件数を数えます。10 件中 9 件が一致すれば 0.9 です。0.948 は、平均して 10 件中 9.5 件が正解と一致したことを意味します。

精度がほとんど落ちない理由は、int8 でも 1 要素を -127 から 127 の 255 段階で持てるからです。元の値との差は小さく、内積の大小関係はほぼ変わりません。そのため int8 は、このあとの Stage 5 のような補正なしで使えます。
