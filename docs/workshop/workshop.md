...
量子化の手法はいくつかありますが、今回は最も単純なスカラ量子化を採用します(ゼロ点を持たない対称型)。ベクトルごとに絶対値の最大値 (maxAbs)を測り、scale = maxAbs ÷ 127 とscaleを計算し、各要素を round(v ÷ scale) で整数(-127〜127)に丸めます。復元も簡単で q × scale で計算できます。他の量子化手法は[付録の「ベクトル量子化の系譜」](../appendix/appendix.md#9-ベクトル量子化の系譜基本から最新まで)で整理しています。
