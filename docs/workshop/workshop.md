...
ここの倍率 4.5x は「naive が上限からどれだけ遠かったか」で決まる量で、CPU によって変わります(naive が既に上限の近くにいる機械なら 1.x 倍程度、遠い機械なら数倍)。変わらないのは「最後はメモリ帯域の上限で頭打ち」という事実のほうです。自分の Codespace で何倍になるかは `make bench1` で分かります。
...
本ワークショップは、性能の上限に当たり、手を打って越え、次の上限に当たる、を繰り返してきました。その変遷と各段での SIMD の役割を表にまとめます。
