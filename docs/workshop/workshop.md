...
算術強度を上げる 2 つ目の方法は、運ぶバイト数を減らすことです。DB ベクトルの各要素を fp32(4 byte)から int8(1 byte)に変換します。この、値を少ないビット数で表し直すことを量子化と呼びます。運ぶ量が 1/4 になるので、算術強度は 0.5 から 2 flop/byte になり、リッジを越えて演算律速側に移ります。整数の計算なので flop とは呼びませんが、回数の数え方は同じなので、図では Gop/s と表記します。

量子化の手法はいくつかありますが、今回は最も単純なものを採用します。ベクトルごとに絶対値の最大 maxAbs を測り、scale = maxAbs ÷ 127 を 1 個だけ覚えて、各要素を round(v ÷ scale) の整数(-127〜127)に丸めます。復元は q × scale です。内積は int8 のまま整数で計算して、最後に 2 本ぶんの scale を 1 回掛ければ済みます。他の手法(コードブックで置き換える PQ、ランダム回転してから量子化する RaBitQ や TurboQuant など)は[付録の「ベクトル量子化の系譜」](../appendix/appendix.md#9-ベクトル量子化の系譜基本から最新まで)で整理しています。
