...
完全な実装は [`internal/vec/dot_portable.go`](../../internal/vec/dot_portable.go) にあり、実測でも archsimd 版と同じ速さになります(`make bench-portable`)。それでも本ワークショップが `archsimd` で進めるのは、ポータブル API にはどのアーキでも共通に持てる演算しか無く、この後の Stage 2 で使う int8 用の命令や popcount が入っていないためです。
