...
まず、1 個ずつの内積を 8 個まとめて処理するように置き換えます。`Float32x8` でスライスから 8 要素をベクトルレジスタにロードし、`MulAdd`(FMA)でアキュムレータに足し込みます。アキュムレータは途中結果をためておく変数のことで、スカラ版の `sum` にあたるのが下のコードの `acc` です。
...
`archsimd.ClearAVXUpperBits()` は、Intel の CPU 向けの後始末です。中身は VZEROUPPER という命令 1 つで、SIMD で使ったレジスタの上位半分をゼロに掃除します。これを置かないと、SIMD からスカラの計算に戻る境界で大きく遅くなることがあります。なぜ遅くなるかは少し上級向けなので、このワークショップでは解説しませんが、詳しくは[付録](../appendix/appendix.md#1-go-の-simd-の性能上限に関する調査)で説明しています。
