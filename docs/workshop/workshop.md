...
ルーフラインの「どの上限にも未達なら命令並列性を上げる」に従い、同時に複数を処理する SIMD で命令並列性を上げ、達成性能を引き上げます。

### Stage 1 — AVX2 で SIMD化(2段階で理解する)

Stage 0 は命令並列性ゼロで上限にも未達でした。ルーフラインの指示は「命令並列性を上げて達成性能を上げる」です。SIMD 化では 2 つの工夫を一度にやりがちなので、混乱しないよう 1a「8 個まとめて読む」、1b「待ち時間を隠す」の順に分けて理解します。
