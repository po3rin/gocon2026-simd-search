...
> ```
>
> どちらも workers を 4 にしても 4 倍にはなりません。止まる場所は 2 つで別です。
>
...
>
> バッチ(B=32)は 1.9x で止まりました。この Codespace の 4 vCPU は物理コア 2 個に SMT で 2 スレッドずつ載せたもので、`lscpu` で確認できます。SMT は 1 つの物理コアを 2 つの CPU として見せる仕組みで、Intel の Hyper-Threading と同じものです。同じ物理コアの 2 スレッドは FMA の実行ユニットを共有するので、計算で詰まっている処理は物理コアの数(2)までしか速くなりません。物理コアが 4 個以上の機械なら、こちらはコア数に応じて伸びます。
>
