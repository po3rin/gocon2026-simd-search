...
**対象と前提:** Go の基礎が読めれば十分で、CPU アーキの予備知識は要りません(専門用語は初出のところで補足します)。**読み方:** 上から順に通読でき、各節は「なぜ → コード → どうなったか」を実行コマンドつきで追えます。引用ブロックのコラムと最後の「持ち帰り」は発展的な内容なので、当日の40分に収まらなくても後から読めます。
...
### 持ち帰り — この先へ進む3つの扉(このリポでは未実装)

本編の道具立て(ルーフライン × 「AI を上げる2つの道」)は、そのまま先へ延長できます。

- **int8 量子化** — float32 を int8 に潰せば転送 4→1 byte で AI が4倍。バイナリ(1bit)ほど精度を落とさず、Faiss / Qdrant の scalar quantization と同じ設計です。Go 1.26 の archsimd には変換(`Float32x8.ConvertToInt32` → `Int32x8.SaturateToInt8`)と int8 の積和(`DotProductPairsSaturated` = VPMADDUBSW / `DotProductPairs` = VPMADDWD)が既にあり、**今日の API で実装できます**(VNNI 系の積和累算命令はまだ未提供)。
- **量子化 × バッチの合成** — Stage 2(①再利用)と Stage 3(②バイト削減)は排他ではありません。int8 化した DB ベクトルをバッチで回せば AI は掛け算で上がり、さらに演算律速側へ。実運用のベクトル検索エンジンが速いのはこの合成のためです。
