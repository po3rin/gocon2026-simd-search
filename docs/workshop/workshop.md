...
> ```
>
> 256bit ではポータブル版も archsimd 版と同じ位置に来ます。書き方が変わっても、ルーフライン上の位置は同じです。幅を 128bit に半分にすると、カーネルは 57 ns から 107 ns と約 2 倍遅くなり、全探索も 8.5 ms から 11.9 ms になりました。理由は 1 ベクトルあたりの時間で分かります。DB ベクトル 1 本は 1536 byte で、DRAM から運ぶのに 1536 ÷ 19 GB/s で約 80 ns かかります。256bit のカーネル(57 ns)は 80 ns より短いので、運ぶ時間で全体が決まります。128bit のカーネル(107 ns)は 80 ns より長いので、今度は計算する時間で全体が決まります。**幅を広げても運ぶ時間は縮まらず、狭めると計算の時間が運ぶ時間を超えて遅くなる。**これがメモリ律速の意味です。Apple Silicon は元が 128bit なので、下 2 行は同じ数字になります。ポータブル API には Stage 3 の int8 用の命令や popcount のようなアーキ固有の命令は入っていません。どのアーキでも共通に持てる演算だけです。そのため本編の量子化カーネルは `archsimd` のままです。

### Stage 2 — クエリのバッチ化：演算律速にして SIMD を効かせる(exact)

全探索がメモリ律速なのは、DB ベクトル 1 本を DRAM から運んでも、それを使う計算が内積 1 回(要素あたり 2 flop)しかないからです。運んだ 1 本を B 本のクエリ全部と内積すれば、運ぶ量は同じで計算が B 倍になり、算術強度は 0.5 × B になります。B = 32 なら 16 で、リッジ(1.3)を越えて演算律速側に移ります。そこでは SIMD が効く見込みです。クエリごとの計算は Stage 1 と同じなので、結果は正確なままです。これは行列と行列の掛け算(GEMM)と同じ構造で、数値計算ライブラリや Faiss のバッチ検索が速いのも同じ理由です。
...
B=32 でクエリを束ねると算術強度は 0.5 から 16 になり、リッジを越えて演算律速側に移りました。そこでは **SIMD がスカラより 5.9x 速くなります**。scalar batch が 34.2 ms/query、SIMD batch が 5.79 ms/query で、GFLOP/s は 2.24 から 13.3 です。Stage 1 では 4.5x で頭打ちだった SIMD が、算術強度を上げると効きます。1 クエリあたりの時間も 7.9 ms から 5.8 ms に縮みます。結果は Stage 1 と同じで正確なままです。

演算ピークは spill のせいで 25.5 GF なので、SIMD batch の 13.3 GF はその 52% です。それでもスカラの 5.9 倍です。この倍率も CPU によって変わります。

「クエリが 32 本まとめて来る」は都合のよい仮定ではありません。ColBERT のようにクエリを複数のベクトルで表す検索方式(late interaction)では、DB ベクトル 1 本に対して複数の内積を取ることが方式そのものに含まれていて、最初から演算律速です。付録 A で実測しています(5.7x)。
...
並列化にも上限があります。メモリ律速ならマシン全体のメモリ帯域、演算律速なら物理コア数で、どちらも 1 コアのルーフラインの外側にある別の上限です。何律速かが先に分かっていれば、goroutine を足して効くかどうかは足す前に予測できます。

### Stage 3 — int8 量子化(もう一つの道：バイトを削る・その1)

算術強度を上げる 2 つ目の道、バイト削減に入ります。§04 の表でいう 2 番目の手を、再利用とは別のやり方で打ちます。DB ベクトルを fp32(要素あたり 4 byte)から int8(1 byte)に変換して持ちます。これを量子化と呼びます。運ぶ量が 1/4 になるので、算術強度は 0.5 から 2 flop/byte になり、リッジ(1.3)を越えて演算律速側に移る見込みです。int8 の計算は整数ですが、回数の数え方は flop と同じなので、図では Gop/s と表記します。距離の計算は整数の掛けて足すなので、量子化しても **SIMD が主役のまま**です。
...
int8 同士の積は最大 127 × 127 で、int32 に余裕で収まります。そのため桁あふれの対策なしで書けます。1 命令で 16 要素を処理でき、fp32 の 8 要素の 2 倍です。arm64 の Neon には VPMADDWD にあたる命令が無いので、`int8_arm64.go` では SMULL で掛けて int16 に広げ、SXTL で int32 に広げてから足す、という 3 段で同じ計算をします。どの命令があるかがアーキごとに違う、それが `archsimd` をアーキ固有と呼ぶ理由です。
...
カーネルは 364 ns から 34.6 ns(**10.5x**)で、fp32 の SIMD カーネル(55 ns)より速くなりました。全探索は 4.2 ms で、fp32 の SIMD 全探索の約 2 倍の速さです。精度は Recall@10 = 0.948 でした。Recall@10 は、正確な検索の上位 10 件のうち、この検索でも上位 10 件に入った割合です。1 バイトでも値の大きさが残るので、そのまま使える精度です。
...
距離の計算は通常の(SIMD でない)POPCNT 命令で足ります。AVX-512 の SIMD 版 popcount(VPOPCNT)を使っても速くなりません。データがキャッシュにあり、1 ベクトルが 6 語と小さいので、popcount で時間を使っていないからです。AVX-512 のある機械で測っても、SIMD 版の `SearchBinarySIMD` が 0.75 ms、通常版の `SearchBinary` が 0.68 ms でした(今回の Codespace の AMD CPU には AVX-512 の VPOPCNT が無く、この付録は動きません)。Stage 3 では SIMD が主役でしたが、1bit では使う場所がありません。計算で詰まっていない所では SIMD は効きません。本編に AVX-512 の Stage を置かないのはこのためで、AVX-512 の話は付録 B にまとめてあります。
...
int8 では保てた精度が、1bit では大きく落ちました。速さは保ったまま精度を取り戻します。ここで SIMD の内積が再び主役になります(Stage 5)。
