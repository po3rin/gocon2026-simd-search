...
完全な実装は [`internal/vec/dot_portable.go`](../../internal/vec/dot_portable.go) にあり、実測でも archsimd 版と同じ速さになります(`make bench-portable`)。それでも本ワークショップが `archsimd` で進めるのは、ポータブル API にはどのアーキでも共通に持てる演算しか無く、この後の Stage 3 で使う int8 用の命令や popcount が入っていないためです。
...
SIMD とベクトル検索の概要が分かったところで、一度動かして現状の速さを測ります。環境がまだの人は [setup.md](setup.md) を先に済ませてください。推奨は GitHub Codespaces で、ローカル(amd64 Linux / Windows、Apple Silicon の Mac)で動かす手順も同じページにあります。
...
この回は約 35.9 ms で、実行ごとに 35〜36 ms 程度でばらつきます。 GFLOP/s と AI(算術強度)の意味はこの後 §04〜05 で説明するので、いまは「そういう列がある」と分かれば十分です。本ワークショップがこれからやることは 1 つ、**この全探索をどう速くするか**です。

「SIMD で並列計算すれば速くなる」と思うところですが、やみくもに SIMD を足しても早くなることは少ないです。そこで本ワークショップは、いきなり速くしようとせず、先に「何で詰まっているか」を測ってから対処を選びます。そのための道具がルーフラインモデルです。
...
3 つの数字が出ました。これがこのマシンの性能の上限です(演算ピーク 約 25.5 GFLOP/s、メモリ帯域 約 20 GB/s)。

補足が 1 つあります。演算ピーク 25.5 GFLOP/s は、この CPU の理論値(約 110 GFLOP/s)の 1/4 に留まります。ハードの限界ではなく、Go のコンパイラがアキュムレータ(途中結果をためる変数)の値をレジスタに置いたままにできず、FMA のたびにメモリ上の作業領域(スタック)と往復させてしまうためです。これを **register spill** と呼びます。機械語を見ながらの説明は[付録の「隠れた上限②: なぜ FMA は理論ピークの 1/3〜1/4 で止まるか(register spill)」](../appendix/appendix.md#隠れた上限②-なぜ-fma-は理論ピークの-1314-で止まるかregister-spill)にあります。
...
CPU チップの理論値はもっと上ですが、いま動かしている Go コードでは 25.5 が現実的な上限です。ルーフライン図には理論値も載せますが、ここでは実測の数値を使います。
...
図: スカラ、SIMD(メモリ帯域の上限)、int8(リッジの右)、1bit 量子化(右上へ)。各点の詳細は下の各節で。バッチ(B=32)の点だけは付録の Stage 2 で扱う。
...
make roofline-batch  # 付録 7 節(Stage 2): バッチ化の比較
...
1. 再利用。運んだ DB ベクトル 1 本を、複数のクエリで使い回す。運ぶ量は同じで計算が増える。結果は正確なまま(付録の Stage 2)
...
### Stage 2 は付録へ: クエリのバッチ化(再利用)

再利用の実演が Stage 2 のクエリのバッチ化です。クエリを 32 本まとめて持ち、DB ベクトルを 1 本運ぶたびに 32 本全部と内積を取ると、運ぶ量はそのままで計算が 32 倍になり、算術強度は 0.5 から 16 に上がってリッジを越えます。演算律速側では SIMD がスカラの 5.9x 効き、結果は正確なままです。

ただしバッチ化は、クエリが 32 本まとめて手元にあるときに使える方法です。クエリが 1 本ずつ来る本編の設定では使えないので、実測と時間内訳は[付録の「7. Stage 2: クエリのバッチ化」](../appendix/appendix.md#7-stage-2-クエリのバッチ化再利用で算術強度を上げる)に置き、本編はもう 1 つの方法、運ぶバイト数を減らす方向で進みます(Stage 番号はコードやベンチ名と揃えるため、2 を欠番にしています)。goroutine 並列がどの上限に効くかの実測も[付録の 8 節](../appendix/appendix.md#8-goroutine-で並列化すればいいのではmake-bench-parallel)にあります。

### Stage 3: int8 量子化(バイトを減らす、その 1)

算術強度を上げる 2 つ目の方法は、運ぶバイト数を減らすことです。DB ベクトルの各要素を fp32(4 byte)から int8(1 byte)に変換して持ちます。値を少ないビット数で表し直すことを量子化と呼びます。運ぶ量が 1/4 になるので、算術強度は 0.5 から 2 flop/byte になり、リッジを越えて演算律速側に移る見込みです。距離の計算は int8 の掛けて足すで、これも SIMD 命令があります。整数の計算なので flop とは呼びませんが、回数の数え方は同じなので、図では Gop/s と表記します。
...
図: Stage 3 の位置。int8 で 算術強度 0.5 から 2 と右へ動き、リッジを越えた。ただし演算ピークの下(内積の実装の速さで頭打ち)で止まる。
...
距離には、内積の代わりにハミング距離を使います。2 つのビット列で値が違うビットの数のことで、XOR で違うビットを 1 にし、1 の数を数える(popcount)だけで求まります。popcount は 1 命令で 64 次元ぶんを数えるので、距離の計算も int8 よりずっと軽くなり、Stage 3 で見えた内積計算の上限も同時に越えられます。
...
同じ出力に並んでいる `binary+rerank=0.868` は、次の Stage 5 で精度を取り戻した結果です。
...
### Stage 5: fp32 SIMD で rerank

Stage 4 は速いものの、Recall 0.18 では使えません。速さは 1bit のまま、精度だけ取り戻します。検索を 2 段に分けます。

1. 1bit のハミング距離で 10 万件すべてを比べ、近い順に上位 100 件(返したい 10 件の 10 倍)を残す。Stage 4 と同じ処理で、0.77 ms
...
![Stage 5 の手順: 1bit で 100 件に絞り、fp32 で採点し直して上位 10 件を返す](../images/rerank.png)
...
| 1bit + rerank(Stage 5) | 0.82 ms  | 0.868        | 速さ最優先。精度は rerank で復元 |


※ 速度は 10 万件のベンチ、Recall@10 は 2 万件の合成データ(Stage 3 の注)で測った値です。
...
- クエリを複数のベクトルで表す late interaction では、付録の Stage 2(バッチ化)と同じ構造がとれるため、最初から演算律速です。[付録の MaxSim](../appendix/appendix.md#2-maxsim) で実装と実測をしています
...
| Stage 1→2(付録) | メモリ帯域(DRAM)     | バッチで 算術強度 0.5→16(再利用) | 効いた(演算側でスカラの 5.9x)      | 5.8 ms/query・exact       |
| コラム(並列)   | 帯域 / 物理コア       | goroutine 並列          | 対象外(SIMD とは別の軸)         | 1.8x / 1.9x で頭打ち         |
| Stage 1→3 | メモリ帯域(DRAM)     | バイト削減 1/4(int8)       | 効いた(int8 内積 10.5x)      | 4.2 ms・Recall 0.948      |
| Stage 3→4 | (int8 では)内積の計算  | バイト削減を極限 1/32(1bit)   | 効かない(popcount はスカラで足りる) | 0.77 ms(46x)…Recall 0.18 |
