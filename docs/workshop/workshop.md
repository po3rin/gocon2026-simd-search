...
検索の中心は 10 万回の内積でした。まず SIMD を使わない内積から始めます。次の Go コードがそれです。
...
未対応の CPU で呼ぶと panic するので、実行時に機能フラグでガードします。x86 系の機能フラグは `archsimd.X86` にまとまっています。
...
`archsimd` とは別に、アーキに依存しない書き方ができる [`simd`](https://pkg.go.dev/simd) パッケージも 1.27 で入りました。型名からレーン数が消え(`Float32x8` ではなく `Float32s`)、幅は実行時に CPU が決めます(AVX-512 機なら 16 レーン、AVX2 機なら 8、Apple Silicon の Neon なら 4)。同じソースが amd64 / arm64 / Wasm で動き、命令の無い環境では純 Go でエミュレートされます。さきほどの内積はこう書けます。
...
上限は 3 つの数字だけで計算できます(§05 で実際に計算します)。この式を横軸に算術強度、縦軸に性能で描くと屋根の形になるので roofline と呼ばれます。図にすると「どちらの上限に近いか」が見やすくなるので、本ワークショップでも図で見ます。コードが遅いとき、原因は大きく 2 つに分かれます。
...
よって今回ナイーブな計算方法だと算術強度は次の計算式になります。

```text
算術強度 = 2 flop / 4 byte = 0.5 flop/byte
```

ここに自分のコードの達成性能(GFLOP/s)と算術強度を当てはめると、点の位置は 3 通りに分かれ、それぞれ対処が違います。
...
一度動かして、現状の速さを測ります。環境がまだの人は [setup.md](setup.md) を先に済ませてください。推奨は GitHub Codespaces で、ローカル(amd64 Linux / Windows、Apple Silicon の Mac)で動かす手順も同じページにあります。
...
実際の画面には、ns 単位の時間と指標が次のように並んで出ます。

```text
35856977 ns/op  4283 MB/s  0.5 AI(flop/byte)  2.142 GFLOP/s  153.6 MB/query
```

この回は約 35.9 ms で、実行ごとに 35〜36 ms 程度でばらつきます。右側の AI(算術強度)と GFLOP/s は §03 で導入した指標で、ベンチが自動で計算して表示します。本ワークショップがこれからやることは 1 つ、**この全探索をどう速くするか**です。

達成は 2.15 GFLOP/s。この点がルーフラインのどこにあるのか、つまりどの上限の下にいるのかを知るために、次章でこのマシンの上限(演算ピークとメモリ帯域)を測ります。
...
3 つの数字が出ました。これがこのマシンの性能の上限です(演算ピーク 約 25.6 GFLOP/s、メモリ帯域 約 20 GB/s)。

補足が 1 つあります。演算ピーク 25.6 GFLOP/s は、この CPU の理論値(約 110 GFLOP/s)の 1/4 に留まります。ハードの限界ではなく、Go のコンパイラがアキュムレータ(途中結果をためる変数)の値をレジスタに置いたままにできず、FMA のたびにメモリ上の作業領域(スタック)と往復させてしまうためです。これを **register spill** と呼びます。機械語を見ながらの説明は[付録の「隠れた上限②: なぜ FMA は理論ピークの 1/3〜1/4 で止まるか(register spill)」](../appendix/appendix.md#隠れた上限②-なぜ-fma-は理論ピークの-1314-で止まるかregister-spill)にあります。

3 つのベンチが具体的にどんなコードで数字を出しているかは[付録の「5. 上限ベンチの中身」](../appendix/appendix.md#5-上限ベンチの中身演算ピークとメモリ帯域の測り方)にまとめてあります。測り方が気になったら、本編のあとに読んでみてください。
...
CPU チップの理論値はもっと上ですが、いま動かしている Go コードでは 25.6 が現実的な上限です。ルーフライン図には理論値も載せますが、ここでは実測の数値を使います。
...
全探索は、どれだけ上手く書いても **10 GFLOP/s 前後**が上限という見込みが立ちます。最初のベースライン(`make bench0`)の 2.15 GFLOP/s は、この上限にすらまだ届いていなかったことになります(あとで SIMD 化すると、この上限のすぐ下 9.7 まで来て止まるのが見れます)。

これでこのマシンの上限(メモリ帯域 約 20 GB/s、演算ピーク 25.6 GFLOP/s)が求まりました。このあと、改良を 1 つ足すたびに出る数字を、この上限と突き合わせていきます。特に「SIMD 化しても約 10 GFLOP/s で止まる」という見積もりは、次章の Stage 1 で実測と突き合わせます。コードを 1 行も直す前に結果を予測できるのが、ルーフラインの価値です。
...
ここからは実際にコードを動かして出る数字を見るパートです。 `internal/vec` / `internal/index` の実コードを使っていきます。手元で追うときは、各節のあとに対応するコマンドを走らせます。

```bash
make bench0          # Stage 0: スカラ基準
make bench1          # Stage 1: SIMD 化(内積単体 + 全探索の2粒度)
make roofline-batch  # 付録 7 節: クエリのバッチ化の比較
make bench-parallel  # 付録 8 節: goroutine 並列はどの上限に効くか
make bench-int8      # Stage 2: int8 量子化(1/4)
make recall-int8     # Stage 2: int8 の精度(Recall@10)
...
この待ち時間で、内積 1 回(384 要素)の実測 348 ns がほぼ説明できます。命令 1 つにかかるサイクル数は CPU の世代(マイクロアーキテクチャ)ごとに決まっていて、このマシンの EPYC 7763 は AMD の Zen3 という世代です。
...
§03 の表でいうと、点はどちらの上限からも遠い 1 番目の位置です。対処は「コードの並列度を上げる」で、その代表が同時に複数の値を計算する SIMD です。次の Stage 1 で SIMD 化します。

### Stage 1: AVX2 で SIMD 化

ここから SIMD 化します。1a「まとめて読む」、1b「待ち時間を隠す」の 2 段に分けます。

#### Stage 1a: まとめて読む(Float32x8)

まず、1 個ずつの内積を 8 個まとめて処理するように置き換えます。`Float32x8` でスライスから 8 要素をベクトルレジスタにロードし、`MulAdd`(FMA)でアキュムレータに足し込みます。アキュムレータ 1 本の、いちばん短い形です。
...
内積の関数だけを測ると 348 ns から 55 ns(6.3x)、全探索も 35.7 ms から 7.9 ms(4.5x)速くなりました。倍率が違うのはデータの居場所の差です。内積単体のベンチは同じ 2 本(計 3 KB)を測り続けるのでデータが L1 キャッシュに載ったままですが、全探索は 154 MB を DRAM から運びます。DB 件数を振ってこの境界を実測した表は[付録の「6. SIMD が効く境界はデータサイズの軸にもある」](../appendix/appendix.md#6-simd-が効く境界はデータサイズの軸にもあるmake-bench-nsweep)にあります。

SIMD は確かに効いています。ただし見るべきは「どこで止まったか」です。全探索の達成は **9.7 GFLOP/s**。これは §05 で見積もったメモリ帯域の上限 0.5 × 約 20 ≈ 10 GFLOP/s の直下で、帯域に直すと 19.4 GB/s で、メモリ帯域の上限 20.8 GB/s の約 93% です。算術強度 0.5 のまま、メモリ帯域のほぼ上限に達しました。
...
1. 再利用。運んだ DB ベクトル 1 本を、複数のクエリで使い回す。運ぶ量は同じで計算が増える。結果は正確なまま(付録のバッチ化)
2. バイト削減。DB ベクトルを小さい型で持ち、運ぶ量そのものを減らす。結果は近似になる(Stage 2 の int8、Stage 3 の 1bit)

> **コラム: 端数はマスク付きロードでも書ける**
>
> 8 個ずつ処理するループは、配列の長さが 8 で割り切れないと最後に 8 個未満の要素が余ります。この余りを端数と呼びます。上のコードは端数をスカラループで処理しています(本編は 384 次元で割り切れるため、実は一度も通りません)。Go 1.27 には端数専用の `LoadFloat32x8Part` があり、足りないレーンをゼロで埋めた 8 レーンのレジスタと読めた要素数を返してくれる(マスク付きロード)ので、端数もベクトルのまま処理できます(1.26 では `LoadFloat32x8SlicePart` という名前でした)。
>
> ![端数のマスク付きロード: 足りないレーンはゼロ埋めされる](../images/masked-load.png)

### クエリのバッチ化は付録へ(再利用)

再利用の実演がクエリのバッチ化です。クエリを 32 本まとめて持ち、DB ベクトルを 1 本運ぶたびに 32 本全部と内積を取ると、運ぶ量はそのままで計算が 32 倍になり、算術強度は 0.5 から 16 に上がってリッジを越えます。演算律速側では SIMD がスカラの 5.9x 効き、結果は正確なままです。

ただしバッチ化は、クエリが 32 本まとめて手元にあるときに使える方法です。クエリが 1 本ずつ来る本編の設定では使えないので、実測と時間内訳は[付録の「7. クエリのバッチ化」](../appendix/appendix.md#7-クエリのバッチ化再利用で算術強度を上げる)に置き、本編はもう 1 つの方法、運ぶバイト数を減らす方向で進みます。goroutine 並列がどの上限に効くかの実測も[付録の 8 節](../appendix/appendix.md#8-goroutine-で並列化すればいいのではmake-bench-parallel)にあります。

### Stage 2: int8 量子化(バイトを減らす、その 1)

算術強度を上げる 2 つ目の方法は、運ぶバイト数を減らすことです。DB ベクトルの各要素を fp32(4 byte)から int8(1 byte)に変換します。このように、値を少ないビット数で表し直すことを量子化と呼びます。運ぶ量が 1/4 になるので、算術強度は 0.5 から 2 flop/byte になり、リッジを越えて演算律速側に移ります。整数の計算なので flop とは呼びませんが、回数の数え方は同じなので、図では Gop/s と表記します。

量子化の手法はいくつかありますが、今回は単純な対称スカラ量子化を採用します。ベクトルごとに絶対値の最大 maxAbs を測り、scale = maxAbs ÷ 127 を 1 個だけ覚えて、各要素を round(v ÷ scale) の整数(-127〜127)に丸めます。復元は q × scale です。内積は int8 のまま整数で計算して、最後に 2 本ぶんの scale を 1 回掛ければ済みます。他の手法(コードブックで置き換える PQ、ランダム回転してから量子化する RaBitQ や TurboQuant など)は[付録の「ベクトル量子化の系譜」](../appendix/appendix.md#9-ベクトル量子化の系譜基本から最新まで)で整理しています。

![int8 の対称スカラ量子化: fp32 の各要素を scale で割って -127〜127 の整数に丸める](../images/int8-quantization.png)
...
ここで初めて精度の数字が出てきました。ここまでの高速化は計算のやり方を変えただけなので、検索結果は正確なままでした。量子化は値そのものを変えるので、ここからは検索結果が fp32 の正解とずれる可能性があります。速くなっても間違った文書を返すのでは意味が無いので、速度と並べて「正解とどれだけ一致しているか」を測ります。その指標が Recall@10 で、次のように測ります。まず fp32 で正確に検索して上位 10 件を出し、これを正解とします。次に int8 で検索して上位 10 件を出し、そのうち正解に含まれていた件数を数えます。10 件中 9 件が一致すれば 0.9 です。0.948 は、平均して 10 件中 9.5 件が正解と一致したことを意味します。正解の取り逃しを測るので Recall と呼びます(正解も取得も同じ 10 件なので、Precision@10 と数値は一致します)。なお Recall の計測は速度のベンチとは別のデータで行っています。意味の近い文書がかたまりを作るように生成した 2 万件の合成データです([`internal/index/index_test.go`](../../internal/index/index_test.go))。

![Recall@10 の測り方: 正解の上位 10 件と何件一致したか](../images/recall.png)

精度がほとんど落ちない理由は、int8 でも 1 要素を -127 から 127 の 255 段階で持てるからです。元の値との差は小さく、内積の大小関係はあまり変わりません。

ただし、運ぶ量を 1/4 にしたのに速度は 4 倍になっていません。達成した帯域は 9.2 GB/s で、メモリ帯域の上限(約 20 GB/s)のずっと下です。算術強度 2 でリッジの右に移ったので、時間を決めているのがメモリから内積の計算に変わったためです。1 つの上限を越えると、次の上限が見えてきます。

#### 次の一手

バイト削減にはまだ先があります。DB 全体がキャッシュに収まるまで減らすとどうなるか。1 要素 1bit(1/32)まで減らします。

### Stage 3: バイナリ量子化(バイト削減を極限まで)

int8 で 1/4 にしても、10 万件 × 384 byte = 38.4 MB で、L3 キャッシュには収まりません。そこで各要素を符号 1bit(正なら 1、負なら 0)だけにします。
...
距離には、内積の代わりにハミング距離を使います。2 つのビット列で値が違うビットの数のことで、XOR で違うビットを 1 にし、1 の数を数える(popcount)だけで求まります。popcount は 1 命令で 64 次元ぶんを数えるので、距離の計算も int8 よりずっと軽くなり、Stage 2 で見えた内積計算の上限も同時に越えられます。
...
同じ出力に並んでいる `binary+rerank=0.868` は、次の Stage 4 で精度を取り戻した結果です。
...
Stage 3 は速いものの、Recall 0.18 では使えません。速さは 1bit のまま、精度だけ取り戻します。検索を 2 段に分けます。
...
これが実際のベクトル検索エンジンでよく使われる構成です。量子化で粗く絞り、精度の高い距離で採点し直す。この 2 段階の検索は実践でも使われます。
...
本編で使った考え方(ルーフラインと、算術強度を上げる 2 つの方法)は、そのまま先へ延長できます。方向は 3 つあります。
...
- 量子化にも int8 と 1bit の先があります。PQ、ScaNN、RaBitQ、TurboQuant といった系譜を[付録の「ベクトル量子化の系譜」](../appendix/appendix.md#9-ベクトル量子化の系譜基本から最新まで)で本編の軸のまま整理しています
- クエリを複数のベクトルで表す late interaction では、付録のバッチ化と同じ構造がとれるため、最初から演算律速です。[付録の MaxSim](../appendix/appendix.md#2-maxsim) で実装と実測をしています
