...
make bench1          # SIMD 化(カーネル単体 + 全探索の2粒度)
make roofline-batch  # バッチ化の比較
make bench2          # 量子化
make bench3          # 仕上げ(rerank の速度)
make bench-parallel  # 寄り道(goroutine 並列はどの天井に効くか)
make recall          # rerank の精度(付録A の int8 も出る)
...
※ 「クエリが B 本まとめて来る」は人工的な仮定ではありません。クエリを複数のトークンベクトルで表す **late interaction(ColBERT 系)** では、文書ベクトル1ロードに対して複数トークンとの内積を取ることが**検索方式そのものに内在**します — つまり最初から演算律速で、SIMD が最初から効く(付録Bで実測: 5.7x)。
...
**なぜ:** Stage 1 の壁からの脱出路は2つありました — **①再利用**(Stage 2 で実測済み)と、ここから入る**②バイト削減**です。ルーフラインの指示は同じく「**算術強度(AI)を上げる＝1バイトあたりの仕事を増やす**」。ただし今度は再利用ではなく、**データ表現そのもの**を変えてバイトを削ります。float32 を**符号1bit**に量子化すると 1ベクトル 1536→48 byte(**1/32**)。距離は内積をやめ、XOR+popcount の**ハミング距離**(違うビット数)へ。
...
※ 1bit は「バイト削減」の**極端な選択**です。中間の **int8(1/4 サイズ)** なら大きさの情報が残って **Recall@10 0.948** と単体で実用域に入り、しかも AI=2 flop/byte でリッジを越えます(付録Aで実装・実測)。本編があえて 1bit を使うのは、精度の崩壊と復元(Stage 4)まで含めて見せるためです。  
...
(出力に並んでいる `binary+rerank=0.868` が、次の Stage 4 でやることの答えです)
...
本編の道具立て(ルーフライン × 「AI を上げる2つの道」)は、そのまま先へ延長できます。int8 量子化と MaxSim は**付録として実装済み**なので、扉はここから3つ:

- **量子化 × バッチの合成** — Stage 2(①再利用)と Stage 3/付録A(②バイト削減)は排他ではありません。int8 化した DB ベクトルをバッチで回せば AI は掛け算で上がり(int8 × B=32 で AI ≈ 64)、さらに演算律速側へ。実運用のベクトル検索エンジンが速いのはこの合成のためです。
...
| Stage 1→3 | **メモリ帯域**(DRAM) | バイト削減 1/32(量子化) | 脇役(popcount はスカラで足りる) | 0.77 ms(46x)…Recall 0.18 |
| Stage 3→4 | **精度**(Recall 0.18) | fp32 rerank | **主役**(SIMD 内積で精度復元) | 0.82 ms・Recall 0.87 |
| 付録A | (int8 では)カーネル演算 | int8 + 整数 SIMD | **主役**(カーネル 10.5x) | 4.2 ms・Recall 0.948 |
...
- **データ表現を変える(量子化)と 46x** — ただし速いのは**データ削減**のおかげで、距離計算に SIMD(VPOPCNT)を足しても速くならない(キャッシュ律速＋小ブロック)。しかも近似で **Recall 0.18 = 単体では使えない**。その精度を **fp32 SIMD の rerank** で 0.87 に戻して初めて実用になる。

まとめると速度は **「AI を上げて演算律速にして SIMD を効かせる」(再利用＝バッチ / バイト削減＝量子化)× 「精度は SIMD 内積」** の二本柱。実運用のベクトル検索エンジン(Faiss / Qdrant / ClickHouse QBit)と同じ設計で、ルーフラインがどちらの軸を攻めるべきかを教えてくれます。**SIMD 抜きでは「速いが使えない(Recall 0.18)」止まり** — 「速くて正確」は、exact なバッチ(Stage 2)と rerank(Stage 4)の **SIMD があって初めて成立**します。

## 付録 — 本編の先へ(実装済み・当日は扱わない)

本編で学んだ読み方を、そのまま別の打ち手・別の検索方式に適用します。すべて実装・実測済みで、コマンド1つで再現できます。

### 付録A: int8 量子化 — 「バイト削減」の現実解

**なぜ:** Stage 3 の 1bit は極端な選択でした(1/32・Recall 0.18)。中間の **int8(1/4 サイズ)** なら大きさの情報が残ります。距離カーネルも popcount ではなく**整数の積和**なので、今度は SIMD が量子化の主役に戻ります。

```go
// internal/vec/int8_simd.go — AVX2 だけで完結(FMA 不要)
...
int8 同士(signed×signed)なら積が最大 127×127 で int32 に余裕で収まり、飽和対策なしの素直なコードで済みます。1命令で **16要素**(fp32 の2倍幅)。

**どうなったか(4コア Codespace 実測):**
...
$ make recall
Recall@10: int8=0.948                                   ← rerank なしで実用域(binary は 0.180)
```

- **AI = 2 flop/byte**(2 flop / 1 byte)で**リッジ(1.3)の右**、つまり演算律速側に乗ります。binary(flop が無く AI を定義できない)と違い、ルーフライン上に点として置ける量子化です。
- ここにも本編と同じ教訓が1つ: 転送を 1/4 にしたのに 4x にはなりません(達成帯域 9.2 GB/s は壁のずっと下)。**壁が遠のいた分、今度はカーネルの演算が新しい律速**になったからです。律速は消えず、移動します。

### 付録B: MaxSim(late interaction) — 最初から演算律速な検索方式
...
### 付録C: AVX-512 VPOPCNT — 「効かない SIMD」の確認用

Stage 3 で述べたとおり、量子化後の Hamming 距離を AVX-512 VPOPCNT(`Uint64x4.OnesCount`)で SIMD 化しても**速くなりません**(キャッシュ律速 + 小ブロック)。`make bench-bonus` で AVX-512 機(AWS c7i 等。`infra/` に Terraform あり)なら再現できます。なお VPOPCNT の無い CPU ではフォールバック分岐のぶん**スカラ版よりむしろ遅くなる**(実測 1.0ms ≧ 0.77ms)ことも含めて、「計算で詰まっていない所に SIMD を足しても効かない」の証拠として残してあります。
