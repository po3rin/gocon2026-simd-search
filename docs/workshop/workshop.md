...
3 つのベンチが具体的にどんなコードで数字を出しているかは[付録の「2. 上限ベンチの中身」](../appendix/appendix.md#2-上限ベンチの中身演算ピークとメモリ帯域の測り方)にまとめてあります。測り方が気になったら、本編のあとに読んでみてください。
...
make roofline-batch  # 付録 4 節: クエリのバッチ化の比較
make bench-parallel  # 付録 5 節: goroutine 並列はどの上限に効くか
...
詰まっているのは `sum += a[i] * b[i]` の足し算です。このループの足し算は、前の `sum` が出来上がらないと次の足し算を始められません。384 回の足し算が 1 本の列になって、1 つずつ順番に待つ形になります。
...
内積の関数だけを測ると 348 ns から 55 ns(6.3x)、全探索も 35.7 ms から 7.9 ms(4.5x)速くなりました。倍率が違うのはデータの居場所の差です。内積単体のベンチは同じ 2 本(計 3 KB)を測り続けるのでデータが L1 キャッシュに載ったままですが、全探索は 154 MB を DRAM から運びます。DB 件数を振ってこの境界を実測した表は[付録の「3. SIMD が効く境界はデータサイズの軸にもある」](../appendix/appendix.md#3-simd-が効く境界はデータサイズの軸にもあるmake-bench-nsweep)にあります。
...
ただしバッチ化は、クエリが 32 本まとめて手元にあるときに使える方法です。クエリが 1 本ずつ来る本編の設定では使えないので、実測と時間内訳は[付録の「4. クエリのバッチ化」](../appendix/appendix.md#4-クエリのバッチ化再利用で算術強度を上げる)に置き、本編はもう 1 つの方法、運ぶバイト数を減らす方向で進みます。goroutine 並列がどの上限に効くかの実測も[付録の 5 節](../appendix/appendix.md#5-goroutine-で並列化すればいいのではへの回答)にあります。
...
量子化の手法はいくつかありますが、今回は最も単純なスカラ量子化を採用します(ゼロ点を持たない対称型)。ベクトルごとに絶対値の最大値 (maxAbs)を測り、scale = maxAbs ÷ 127 とscaleを計算し、各要素を round(v ÷ scale) で整数(-127〜127)に丸めます。復元も簡単で q × scale で計算できます。他の量子化手法は[付録の「いろんなベクトル量子化」](../appendix/appendix.md#6-いろんなベクトル量子化)で整理しています。
...
- 量子化にも int8 と 1bit の先があります。PQ、ScaNN、RaBitQ、TurboQuant といった系譜を[付録の「いろんなベクトル量子化」](../appendix/appendix.md#6-いろんなベクトル量子化)で本編の軸のまま整理しています
- クエリを複数のベクトルで表す late interaction では、付録のバッチ化と同じ構造がとれるため、最初から演算律速です。[付録の MaxSim](../appendix/appendix.md#8-maxsim) で実装と実測をしています
