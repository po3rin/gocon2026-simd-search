...
Go 1.27 の実験的 SIMD で、外部ライブラリなしの Pure Go ベクトル検索を高速化します。ただし闇雲には触りません。ルーフラインモデル(§04 で説明する、性能の上限を図にする方法)を使って「測る、算術強度(AI)を出す、いま何が性能の上限になっているかを見る、その上限に効く手だけ打つ」を繰り返します。

## はじめに

**これは何か:** Go 1.27 の標準 SIMD(`simd/archsimd`)を使い、外部ライブラリなしの Pure Go でベクトル検索を高速化する教材です(Go Conference 2026・40 分ワークショップ)。読みながら手元で動かせます。題材は内積によるベクトル検索です。ただ速くするのではなく、ルーフラインモデルで「いま何が性能の上限になっているか」を測ってから打つ手を選びます。構成は「性能の上限(律速。ボトルネック)に当たる、手を打って越える、次の上限に当たる」の繰り返しで、各段で SIMD がどの役割(主役、脇役、効かない)を果たすかを実測で確かめます。

**何が学べるか:**

- Go の標準 SIMD(archsimd)の書き方。アセンブリも cgo も書かずにベクトル命令を使う
- ルーフラインモデルの使い方。計算とメモリ帯域のどちらが性能の上限かを 1 枚の図で判断し、その上限に効く手だけ打つ
- SIMD がどこで効くか。計算が上限なら効き、メモリ帯域が上限なら頭打ちになる
- 高速化の二本柱。実装効率(SIMD)とデータ表現(バッチ化、量子化)。そして速度と精度の両立(rerank)
- 実測の作法。カーネル単体と全体の 2 粒度で測る、上限そのものをマイクロベンチで測る、Recall で精度を測る
...
Go 1.27 では `GOEXPERIMENT=simd` を付けてビルドすると `simd/archsimd` パッケージが使えます(Go 1.26 で導入、1.27 で API 改訂と arm64 / Wasm 対応)。1.27 でもまだ「実験的」の扱いで、このフラグを付けないとパッケージ自体が存在せず、Go 1 の互換性保証の対象外です(API 名も 1.26 から 1.27 で変わりました)。アセンブリも cgo も書かずにベクトル命令を直接扱え、メソッド呼び出しがほぼそのまま 1 つの CPU 命令にコンパイルされます:
...
注意点として、`archsimd` はアーキテクチャ固有の API です。型も命令も CPU ごとに違います。Go 1.27 時点で対応しているのは amd64 の AVX/AVX2/AVX-512、arm64 の Neon(128bit)、WebAssembly(128bit)です。本編のコードと数字は amd64 の Codespaces で取ったものです。Apple Silicon の Mac でも Go 1.27 からは Neon 版の `internal/vec/dot_arm64.go` が走ります。型は `Float32x4` のような 128bit だけで、Neon は必須機能なのでガードは要りません。ただし幅もメモリ帯域も違うので、出てくる数字は本編とは別物になります(§09)。同じ内積をアーキに依存せず書けるポータブルな `simd` パッケージも 1.27 で入りました。Stage 1 のコラムで使います。
...
「SIMD で並列計算すれば速くなる」と思うところですが、やみくもに SIMD を足しても、最後はメモリ帯域の限界で頭打ちになります。

そこで本資料は、速くする手をいきなり打たず、先に「何で詰まっているか」を測って見極めてから打ち手を選びます。そのための道具がルーフラインモデルです。
...
![メモリ帯域の限界: 計算を速くしてもデータが届かない](../images/memory-wall.png)

図: 本資料のデータ(10 万件 = 153MB)はキャッシュに入りきらず、クエリのたびに DRAM から帯域の上限の速さで運ぶことになる。以降「キャッシュに乗る」「DRAM に溢れる」と言ったら、この階層の話。

下図は、縦軸に性能(GFLOP/s。1 秒あたりの浮動小数点演算回数)、横軸に算術強度(AI。計算量 ÷ データ転送量)を取り、その機械の物理的な上限を線で描いたものです。この線が屋根(roofline)の形をしているのが名前の由来です。左側にメモリ帯域で決まる右上がりの斜線(メモリ律速)、右側に演算ピークで決まる水平線(演算律速)の領域があります。

以降、本資料では次の言葉をこの図の意味で使います。

- **天井**: この上限の線のこと。メモリ帯域で決まる左側の斜線を「メモリ天井」または「メモリの壁」、演算ピークで決まる右側の水平線を「演算天井」と呼びます
- **点を打つ**: 自分のコードの達成性能(GFLOP/s)と算術強度(AI)を測り、この図の上に 1 点として置くこと
- **壁に張り付く**: 打った点がメモリ天井のすぐ下まで来ていること。そのコードはメモリ帯域の上限に達していて、計算を速くしても伸びません
- **リッジ**: 斜線と水平線が交わる点の AI。これより左はメモリ律速、右は演算律速です(§05 で実測から求めます)
...
では、今回使うマシンの天井(メモリ帯域と演算ピーク)はどこにあるのか。次の章で測ります。
...
> ```
>
> レジスタに収まっていれば本体は `VFMADD` だけになります。その前後に必ず付く `VMOVDQU …(SP)` が spill です。この spill が、演算天井が理論ピークの約 100 に届かず 25.5 GFLOP/s に留まる主因とみられます。ただし spill を消せば理論ピークに届くかは、本資料では検証していません。それでも打ち手は変わりません。検索は AI=0.5 のメモリ律速なので、天井がこの低さでも結論は同じです。なぜ 0.65 FMA/cyc なのかというポート圧まで踏み込むなら llvm-mca や uiCA の領域で、本編の 40 分には含めません。arm64 の Neon でも同じ退避が起きます。Apple M3 Pro で 12 本の FMLA ループ `BenchmarkPeakFLOP_NEON` は 32 GFLOP/s 止まりで、`-S` を見ると各 FMLA の前後に `FMOVQ …(SP)` が付きます。アーキが変わっても、Go の SIMD コード生成がまだ発展途上である点は共通です。
...
**結論:** もう実装効率では伸びません。幅を 8 から 16 の AVX-512 に広げても壁は動かず、キャッシュブロッキングも効きません。DB ベクトルは各 1 回しか読まれず再利用が無いので、タイリングしても DRAM 読み出し総量は変わらないからです。次は実装効率ではなく、算術強度(AI)を上げる番です。AI を上げる道は 2 つあります。①再利用(クエリのバッチ化。結果は exact のまま)と ②バイト削減(量子化。結果は近似になる)です。まず①を Stage 2 で、続いて②を Stage 3 の int8 と Stage 4 の 1bit の 2 段で扱います。
...
> ```
>
> 読み方: 256bit ではポータブル版も archsimd 版と同じ点に乗ります。API が変わってもルーフライン上の位置は同じです。幅を 128bit に半分にすると、カーネルは 57 ns から 107 ns と約 2 倍遅くなり、全探索も 8.5 ms から 11.9 ms と遅くなりました。これは 1 ベクトルあたりの時間で読めます。DB ベクトル 1 本は 1536 byte で、DRAM から運ぶのに 1536 ÷ 19 GB/s で約 80 ns かかります。256bit のカーネル 57 ns はこの 80 ns の下に隠れるので壁に張り付きますが、128bit の 107 ns は 80 ns からはみ出すので、律速がメモリからカーネルへ戻ります。**幅は、広げても壁の上には行けず、狭めると壁の下に落ちる。**点は壁の下側にしか動けない、というのがメモリ律速の意味です。Apple Silicon は元が 128bit なので、下 2 行は同じ数字になります。ポータブル API には Stage 3 の int8 積和(VPMADDWD)や popcount のようなアーキ固有の命令は入っていません。どのアーキでも共通に持てる演算だけです。そのため本編の量子化カーネルは `archsimd` のままです。
...
**どうなったか:** B=32 でクエリを束ねると AI は 0.5 から 16 になり、リッジを越えて演算律速側に移りました。そこでは **SIMD が exact のままスカラを 5.9x 引き離します**。scalar batch 34.2 ms/query に対し SIMD batch 5.79 ms/query で、GFLOP/s は 2.24 から 13.3 です。Stage 1 では壁に張り付いて伸びなかった SIMD が、AI を上げた途端に効きます。1 クエリあたりも 7.9 ms から 5.8 ms と速くなります(B=1 比 1.4x)。精度は一切犠牲にしていません。
...
なぜ反転するのかは、時間の内訳で見ると分かります。ルーフラインは 1 要素あたりの時間を、メモリ時間(運ぶ byte ÷ 帯域)と演算時間(flop ÷ 演算ピーク)に分けて考え、実時間はおおよそ長い方になります。演算時間は 2 flop 固定で全 Stage 共通ですが、バッチ化はメモリ時間だけを 1/32 に縮めます。そのため律速がメモリから演算へ反転します。下は §05 で実測した天井、演算 25.6 GFLOP/s と read 帯域 20.8 GB/s から計算したものです。
...
- 演算律速(バッチ)は物理コア数で止まります。この Codespace の 4 vCPU は物理 2 コアに SMT で 2 スレッドずつ載せたもので、`lscpu` で確認できます。SMT の兄弟スレッドは FMA などの実行ユニットを共有するので、演算律速の仕事は物理コア数の 2 ぶんまでしか増えず、1.9x で止まります。物理 4 コア以上の機械なら、こちらはコア数なりに伸びます。メモリ律速側は伸びません。
...
本編の道具立て(ルーフラインと「AI を上げる 2 つの道」)は、そのまま先へ延長できます(MaxSim は付録で実装済み)。方向は 3 つあります:
...
- **アルゴリズムの軸(HNSW・IVF)。** ここまでの手は全て「10 万件全部に触る」前提でした。そもそも触る件数を減らすのが第 3 の軸で、全探索の O(N) を割る唯一の道です。ただし飛び飛びのメモリアクセスになるため、今度は帯域ではなくレイテンシが敵になります。ルーフラインとは別の性能モデルが要る領域で、実運用は「候補を絞る(HNSW/IVF)」と「絞った中を本資料の技で速く走る」の合成です。
- **Go の SIMD はまだ動いている。** Go 1.27 で、ベクトル長に依存しないポータブルな `simd` パッケージ([golang/go#78902](https://github.com/golang/go/issues/78902)。Stage 1 のコラムで使いました)と、`archsimd` の arm64 と WebAssembly 対応が入りました。本資料の `dot_arm64.go` と `int8_arm64.go` はその arm64 対応を使っています。どちらもまだ `GOEXPERIMENT=simd` が要る実験的 API です。amd64 版をデフォルト有効にする提案([golang/go#78979](https://github.com/golang/go/issues/78979))は保留中で、arm64 の可変長ベクトル SVE も未着手です。§05 で見た register spill も 1.27 で残っています。API 名は変わり続けます。1.26 から 1.27 では `LoadFloat32x8Slice` が `LoadFloat32x8` になりました。それでもルーフラインの読み方は、どのアーキテクチャでもどのバージョンでもそのまま使えます。
...
6 つの遷移のうち 4 つで SIMD が突破の実行者です。効かない場所(メモリ壁の正面、量子化後の popcount)がはっきりあるからこそ、効く場所の 4 回が際立ちます。その見極めを与えるのがルーフラインでした。実装効率を上げる(SIMD、アキュムレータ)ことと、算術強度(AI)を上げる(バッチ化、量子化)ことは別の軸です。Stage 1 でメモリ帯域に達した時点で「次は実装効率ではなく AI」と手が決まり、AI を上げた先でまた SIMD が効きました。速度と精度も別の軸で、1bit 量子化で落ちた精度は fp32 SIMD の rerank で取り戻しました。

持ち帰ってほしい作法は次の 6 つです:

1.  **ルーフラインで先に上限を見る。** AI=0.5 はリッジ(約 1.3)を大きく下回る。1 行も書く前から「メモリ律速で、本命は AI を上げること」と分かる
2.  **ベースラインなしに最適化を始めない。** Stage 0 で素朴な数字をまず押さえる
3.  **カーネル単体と全体の 2 粒度で測る。** 2 つを別々に測って、当たる天井を見比べる
4.  **SIMD はデータが届いてこそ。** メモリ帯域が上限のときは、幅を広げても伸びない
5.  **速度と精度を最初から 2 軸で測る。** 量子化と rerank で両立する(Recall 0.18 から 0.87)
6.  **並列もまた上限に当たる。** メモリ律速にコアを足しても帯域で 1.8x 止まり、演算律速は物理コア数まで。コアを足す前に何律速かを見る
...
**Apple Silicon 参加者向け:** Go 1.27 から `archsimd` が arm64 の Neon(128bit)に対応したので、手元の Mac でも SIMD パスが走ります。`//go:build arm64` の `dot_arm64.go` と `int8_arm64.go` がそれで、天井ベンチも Neon 版があります。ただし本編の数字とは別物です。レジスタ幅は 256 から 128bit に半分になる一方、単コアのメモリ帯域は Codespaces より大きいので、点も倍率も本編とは違う位置に打たれます。M3 Pro の実測は次のとおりです。

| 項目 | M3 Pro(Neon) | Codespaces(AVX2) |
|---|---|---|
| read 天井 | 約 34 GB/s | 約 20 GB/s |
| 全探索 naive から SIMD | 34.6 ms から 4.6 ms(7.5x) | 35.7 ms から 7.9 ms(4.5x) |
| binary | 0.43 ms | 0.77 ms |

当日は GitHub Codespaces を共有環境にすれば、手元のアーキ差を気にせず全員が同じ条件で達成性能と AI を測れます。Mac の数字は自分の点として持ち帰ってください。

**CPU について:** 本編で使う SIMD は AVX2 + FMA だけです(Stage 1/2/5 の内積。Stage 3 の int8 カーネルは AVX2 のみ)。過去 10 年の x86(Intel Haswell 2013 以降、AMD 2015 以降)がほぼ全て持つため、Codespaces にどの CPU が当たっても本編は再現します。AVX-512 の VPOPCNT は本編では使いません。Stage 4 で見たとおり量子化後は速くならないためです。AVX-512 を実機で確かめたい人だけ、`infra/` の AWS c7i(Sapphire Rapids)を使ってください。
