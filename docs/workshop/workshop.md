...
やり方は単純です。ベクトルごとに絶対値の最大 maxAbs を測り、scale = maxAbs ÷ 127 を 1 個だけ覚えて、各要素を round(v ÷ scale) の整数(-127〜127)に丸めます。復元は q × scale です。内積は int8 のまま整数で計算して、最後に 2 本ぶんの scale を 1 回掛ければ済みます。
