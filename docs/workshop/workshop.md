...
- ルーフラインモデル。何で詰まっているか(計算が上限か、メモリ帯域が上限か)を 1 枚の図で見極め、測る、算術強度(AI)を出す、いま何が上限かを見る、その上限に効く手だけ打つ
...
- 実測の作法。2 粒度で測る、性能の上限そのものをマイクロベンチで実測する、Recall で精度を測る
