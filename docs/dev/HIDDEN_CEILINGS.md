# Go の SIMD の2つの隠れた性能上限（VZEROUPPER の遷移ペナルティ / register spill）
...
図: 同じコードでも、SIMD からスカラへ戻る境界に `VZEROUPPER` を 1 命令置くかどうかで 167ns と 23ns(7.1x)。左(税あり)は dirty な YMM 上位とレガシー SSE の衝突でペナルティが命令ごとに蓄積し、右(税なし)は `VZEROUPPER` で上位128bitを掃除して衝突そのものを消す。
