...
func HammingSIMD(a, b []uint64) int {
	...
	for len(a) >= 8 {
		acc0 = acc0.Add(archsimd.LoadUint64x4Slice(a).Xor(archsimd.LoadUint64x4Slice(b)).OnesCount())
		acc1 = acc1.Add(archsimd.LoadUint64x4Slice(a[4:]).Xor(archsimd.LoadUint64x4Slice(b[4:])).OnesCount())
		a = a[8:]
		b = b[8:]
	}
	if len(a) >= 4 {
